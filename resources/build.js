...
let js = (await Bun.file("main.js").text()).replaceAll(/(?<!let|new)\s/g, "").replaceAll("$total", total).replace("$orders", orders).replace("elseif", "else if").replace("elsel", "else l").replace("(^|)", "(^| )");
